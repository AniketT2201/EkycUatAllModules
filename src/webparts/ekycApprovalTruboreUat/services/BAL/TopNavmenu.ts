...
export interface ITopNavmenuOps {
    getTopNavmenuData(props: IEkycApprovalTruboreUatProps): Promise<ITopNavmenu[]>;
}

export default function TopNavmenuOps(): ITopNavmenuOps {
    const spCrudOps = SPCRUDOPS();

    const getTopNavmenuData = async (props: IEkycApprovalTruboreUatProps): Promise<ITopNavmenu[]> => {
        try {
