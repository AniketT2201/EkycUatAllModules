...
export interface IDashboardOps {
    getHistoryData(ekycId: number,props: IEkycTruboreUatProps): Promise<IHistory[]>;
    insertHistoryData(item: KYCData, props: IEkycTruboreUatProps): Promise<any>;
...
export default function HistoryOps(): IDashboardOps {
    ...
    const getHistoryData = async (ekycId: number, props: IEkycTruboreUatProps): Promise<IHistory[]> => {
    ...
    const insertHistoryData = async (item: KYCData, props: IEkycTruboreUatProps): Promise<any> => {
        try {
            const spCrudOpsInstance = await spCrudOps;
